...
let seedAdmin = async () => {
  ...
  if(roleAdmin){
    ...
    if(admins.length > 0){
      ...
      try{
        let admin = { roles: [ roleAdmin ], verified : true, status : true, name : seeder.adminName, email : seeder.adminEmail, password : seeder.adminPass, createdAt: new Date(), updatedAt: new Date()  };
